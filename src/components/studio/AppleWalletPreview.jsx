    ...
    <div className="lf-flip-container">
      <div className={`lf-flip-inner ${isFlipped ? 'flipped' : ''}`}>
        ...
        >
          ...
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '40px', background: 'linear-gradient(180deg, rgba(255,255,255,0.1), transparent)', pointerEvents: 'none' }} />

          {/* Top Header */}
          <div style={{ padding: '16px 16px 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 2 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              {restaurant.logoUrl ? (
                <img
                  src={restaurant.logoUrl}
                  alt="logo"
                  style={{ width: '32px', height: '32px', borderRadius: '50%', objectFit: 'cover', border: '1.5px solid rgba(255,255,255,0.3)', boxShadow: '0 2px 8px rgba(0,0,0,0.5)' }}
                  onError={(e) => { e.target.style.display = 'none'; }}
                />
              ) : (
                <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '11px' }}>
                  {restaurant.name?.slice(0, 2).toUpperCase()}
                </div>
              )}
              <div>
                <h3 style={{ fontSize: '13.5px', fontWeight: 800, lineHeight: 1.1, letterSpacing: '-0.02em', color: textColor }}>{restaurant.name}</h3>
                <p style={{ fontSize: '10px', opacity: 0.75 }}>{restaurant.tagline}</p>
              </div>
            </div>

            <div style={{ textAlign: 'right' }}>
              <span style={{ fontSize: '9px', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--font-mono)', opacity: 0.6, display: 'block' }}>POINTS</span>
              <span style={{ fontSize: '16px', fontWeight: 900, fontFamily: 'var(--font-mono)', color: accentColor }}>
          ...
          {restaurant.bannerUrl ? (
            ...
            <div style={{ width: '100%', height: '80px', background: 'rgba(0,0,0,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', opacity: 0.4 }}>
              Pass Strip Banner
            </div>
          )}

          {/* Reward Status & Stamp Matrix */}
          <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', flex: 1 }}>
            
            {/* Reward Title */}
            <div>
              <div style={{ fontSize: '10px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.08em', color: accentColor, marginBottom: '2px' }}>
            ...
            <div style={{ padding: '6px 0' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '10px', fontFamily: 'var(--font-mono)', opacity: 0.6, marginBottom: '6px' }}>
                <span>STAMP MATRIX</span>
                <span>{activeStampsInCycle}/{totalStamps}</span>
              </div>
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '12px' }}>
            ...
            {/* 2. Instagram Direct Link Button */}
            {instagramUrl && (
